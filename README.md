# Image Classifier 
  ...
  In this project, I've trained an image classifier to recognize different species of flowers, for which I've used a dataset of 102 flower categories.
    
  *This is the final Project of the Udacity AI Programming with Python Nanodegree*
...
* Train a new network on a data set with ```train.py```
  ...
  * Options:
    * Set direcotry to save checkpoints: ```python train.py --save_dir save_directory```
    * Choose arcitecture (alexnet, densenet121 or vgg16 available): ```pytnon train.py --arch "vgg16"```
    * Set hyperparameters: ```python train.py --learning_rate 0.002 --hidden_units 4096 --epochs 15 ```
    * Use GPU for training: ```python train.py --gpu gpu```
    
* Predict flower name from an image with ```predict.py``` along with the probability of that name. That is you'll pass in a single image ```/path/to/image``` and return the flower name and class probability
  * Basic usage: ```python predict.py --image /path/to/image```
  * Options:
    * Return top **K** most likely classes:``` python predict.py ---topk 5```
